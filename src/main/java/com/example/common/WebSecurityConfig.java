...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter{
	...
	public void configure(WebSecurity web) throws Exception {
		web.ignoring().antMatchers("/dist/**", "/img/**", "/lib/**","/fonts/**");
	}
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.formLogin() // ログインに関する設定
				.loginPage("/admin/toLogin") // ログイン画面に遷移させるパス(ログイン認証が必要なパスを指定してかつログインされていないとこのパスに遷移される)
				.loginProcessingUrl("/login") // ログイン可否判定するパス（HTMLの入力フォームでth:action()内に指定）
				.failureUrl("/admin/toLogin?error=true") // ログイン失敗に遷移させるパス
				.defaultSuccessUrl("/contracts/top", false) // 第1引数:デフォルトでログイン成功時に遷移させるパス
		...
		http.logout() // ログアウトに関する設定
				...
				.logoutSuccessUrl("/showLogin") // ログアウト後に遷移させるパス(ここではログイン画面を設定)
